...
class NOVA(UnlearnTrainer):
    ...
    def get_soft_target(self, model: nn.Module, forget_inputs: dict) -> torch.Tensor:
        ...
        Args:
            ...
            forget_inputs (dict): A dictionary containing 'input_ids', 'attention_mask',
                                  and 'labels' for the forget set.

        Returns:
            torch.Tensor: The logits generated by the model for the forget inputs,
                          to be used as soft targets.
        """
        # Ensure model is in eval mode if you want consistent, non-training-mode outputs
        # from the model when generating targets.
        ...
        with torch.no_grad(): # No gradients needed for target generation
            outputs = model(
                ...
                # labels=forget_inputs["labels"] # Labels are not needed for generating logits
            )
        
        # Restore model training state
        model.train(original_training_state)

        # Assuming outputs.logits contains the raw logits
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Apply soft targets
        # The soft targets are generated by passing the forget inputs through the model.
        if self.soft_target:
            soft_target = self.get_soft_target(model, forget_inputs)
            forget_inputs["labels"] = soft_target
        # TODO: Currently, this makes soft target used for the noise optimization and the impairment phase.
        #       It might be useful to use soft targets only for the noise optimization, or vice versa.
        ...
        forget_loss, forget_outputs = self.compute_intermediate_loss(model=model, inputs=forget_inputs_processed, uses_embeds=True)
        ...
        retain_loss, retain_outputs = self.compute_intermediate_loss(model=model, inputs=retain_inputs_processed)
